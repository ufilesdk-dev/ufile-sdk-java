...
public class DownloadUrl {
    ...
    public String getUrl(UFileClient uFileClient, UFileRequest request, int ttl, boolean isPrivate) {
        if (!isPrivate) {
            try {
                return "http://" + request.getBucketName() + uFileClient.getUFileConfig().getDownloadProxySuffix()
    ...
    public String makeSignature(UFileClient client, UFileRequest request, long expires) {
        ...
        String signature = new HmacSHA1().sign(client.getUFileConfig().getUcloudPrivateKey(), stringToSign);
        return signature;
    }
    ...
    public String makeDownloadUrl(UFileClient ufileClient,
        ...
        String url = "";
        try {
            url = "http://" + request.getBucketName() + ufileClient.getUFileConfig().getDownloadProxySuffix()
                    + URIUtil.encodePath("/" + request.getKey(), UTF8)
                    + "?" + URIUtil.encodeWithinQuery("UCloudPublicKey", UTF8)
                    + "=" + URIUtil.encodeWithinQuery(ufileClient.getUFileConfig().getUcloudPublicKey(), UTF8)
